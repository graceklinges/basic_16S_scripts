...
#subset your data by specific variables (site, species)
ps_rel <- subset_samples(ps_rel, Site.Name == "Lindsays.Patch" & Objective == "cor" & Sample.Type == "Lesion")

#grouping together taxa that share a genus identification, also removing rare taxa if greater than 2% in samples, in more than 25% of my samples
pseq <- aggregate_rare(ps_rel, level = "Genus", detection = 2/100, prevalence = 25/100)

#reordering variables if that is meaningful for your data (distance to main city?)
...
p <- plot_composition(pseq,
  ...
  labs(x = "Replicate", y = "Relative abundance (%)",
       title = "Genotype 7 Samples from 2020 Bleaching Paper") + 
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text.x = element_text(angle=90, hjust=1),
        legend.text = element_text(face = "italic"))
print(p)  
ggsave(filename="Geno7_barplot.png", plot=p, device="png",  width = 7, height = 6, dpi=500)
