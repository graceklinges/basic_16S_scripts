...
setwd("~/Restoration_microbiome/031621EMillcus515F-243992770") #29 coral tissue samples (25 Amoxicillin-Ampicillin experiment, 4 OFAV Recruits with Tissue Regression)

# load unrarefied, renamed data for differential abundance analysis
load(file = "ps_filt_rename.RData")
sample_sums(ps)

mapfile = "Metadata_031621.txt"
...
ps <- subset_samples(ps, Species == "Goniastrea")

OTUdf <- as.data.frame(t(otu_table(ps)))
metadf <- read.delim(file = "~/Mote_nutrient_experiment/data/Mapping-file-full-renamed.txt") #your metadata
colnames(metadf)[1] <- "Sample.ID" 
metadf$Exposure_weeks <- as.factor(metadf$Exposure_weeks, levels = c("Haqul", "Magna")) #
levels(metadf$Exposure_weeks) #checking order


# Step 1: Data preprocessing, don't change any of this
...
# Step 2: ANCOM, define your models. main_var site?
...
figdf$col_genus[figdf$col_genus != "Family_Helicobacteraceae"] <- NA
...
ggfig <- ggplot(figdf, aes(x = x, y = y, color = col_genus)) +
  ...
  scale_color_manual(name = "Genus", values = colors) +
  geom_hline(yintercept = cut_off["detected_0.7"], linetype = "dashed") +
  annotate("text", label = "W = 0.7", size = 3.5, x = -2, y = 19.75, color = "orange")
ggfig


# #below here is useful if you want to have multiple panels in your final plot
